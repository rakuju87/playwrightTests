...
import { CurrentWeatherDataLatLong } from '../testdata/currentWeatherDataLatLong';
...
CurrentWeatherDataLatLong.forEach(data => {
  test.describe.serial(`WeatherBitAPI - Current weather ${data.tc}`, async () => {
    test.beforeAll(async ({ browser }) => {
      request = (await browser.newContext()).request;
      apiActions = new ApiActions(request);
    });
    test(`Get current weather data`, async () => {
      currentWeatherResponse = await apiActions.currentWeatherDataByLatLong(data.lat, data.long);
    });
